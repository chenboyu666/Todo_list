...
<body class=\"{body_class}\">
  ...
  <script>
    ...
    const TAG_OVERVIEW_RADIUS=Math.min(218,150+tagKeywords.length*8);
    const TAG_OVERVIEW_Y_SPREAD=34;
    const OVERVIEW_TASK_DISTANCE_BASE=36;
    const OVERVIEW_TASK_DISTANCE_STEP=5;
    const OVERVIEW_MORE_DISTANCE=54;
    ...
    const legacySelectedHint = "selected=nodes.find(n=>n.type==='task')||nodes[0]";
    let width=0,height=0,rotX=-.14,rotY=.58,zoom=INITIAL_ZOOM,drag=null,panDrag=null,selected=nodes.find(n=>n.type==='keyword')||nodes.find(n=>n.type==='task')||nodes[0],projected=[];
    ...
    const legacyTargetHint = "target=(a.type==='keyword'||b.type==='keyword')?36:30";
    ...
    function focusAlpha(n) {{ if(graphLayer==='overview'&&n.type==='keyword') return 1; return !selected||selectedNeighborhood(n)?1:.25; }}
    ...
    function updateShortcutHint() {{ shortcutHint.textContent=graphLayer==='tag'?'Esc 返回 · 中键拖动画布 · 滚轮缩放 · 单击任务查看':'单击标签查看全部 · 双击标签进入 · Esc 返回 · 滚轮缩放'; }}
    function enterTagLayer(tag) {{ if(!tagSummaryMap.has(tag)) return; graphLayer='tag'; activeTag=tag; panX=0; panY=0; transitionUntil=frameTick+36; backButton.hidden=false; selected=nodeMap.get(tagNodeId(tag))||selected; setLayerPositions(); renderQueue(selected); updateShortcutHint(); }}
    function returnToOverview() {{ graphLayer='overview'; activeTag=null; panX=0; panY=0; transitionUntil=frameTick+28; backButton.hidden=true; setLayerPositions(); renderQueue(selected); updateShortcutHint(); }}
    ...
    function project(n) {{ const cy=Math.cos(rotY),sy=Math.sin(rotY),cx=Math.cos(rotX),sx=Math.sin(rotX); let x=n.x*cy-n.z*sy,z=n.x*sy+n.z*cy,y=n.y*cx-z*sx; z=n.y*sx+z*cx; const p=620/(620+z); return {{node:n,x:width*.52+x*p*zoom+(graphLayer==='tag'?panX:0),y:height*.55+y*p*zoom+(graphLayer==='tag'?panY:0),z,scale:p*zoom}}; }}
    ...
    canvas.addEventListener('pointerdown',e=>{{ if(e.button===1&&graphLayer==='tag'){{ panDrag={{x:e.clientX,y:e.clientY}}; canvas.setPointerCapture(e.pointerId); e.preventDefault(); return; }} const n=hit(e.clientX,e.clientY); if(n) select(n); drag={{x:e.clientX,y:e.clientY}}; canvas.setPointerCapture(e.pointerId); }});
    canvas.addEventListener('pointermove',e=>{{ if(panDrag){{ panX+=e.clientX-panDrag.x; panY+=e.clientY-panDrag.y; panDrag={{x:e.clientX,y:e.clientY}}; return; }} if(!drag) return; rotY+=(e.clientX-drag.x)*.006; rotX+=(e.clientY-drag.y)*.004; rotX=Math.max(-1.15,Math.min(1.15,rotX)); drag={{x:e.clientX,y:e.clientY}}; }});
    canvas.addEventListener('pointerup',()=>{{ drag=null; panDrag=null; }}); canvas.addEventListener('wheel',e=>{{ e.preventDefault(); zoom*=e.deltaY>0 ? .92 : 1.08; zoom=Math.max(.64,Math.min(2.2,zoom)); }},{{passive:false}});
