...
<body class=\"{body_class}\">
  ...
  <script>
    ...
    tasks.forEach((task, index) => {{ const a = index * 1.72; addNode({{...task,type:'task',color:task.late?'#ff6f91':'#32dcff',radius:task.late?7.2:6.4,x:Math.cos(a)*(76+(index%3)*18),y:Math.sin(index*.91)*44,z:Math.sin(index*1.37)*(66+(index%4)*14),vx:0,vy:0,vz:0}}); }});
    ...
    let width=0,height=0,rotX=-.14,rotY=.58,zoom=INITIAL_ZOOM,drag=null,selected=nodes.find(n=>n.type==='task')||nodes[0],projected=[];
    ...
    function draw() {{ ctx.clearRect(0,0,width,height); const bg=ctx.createRadialGradient(width/2,height/2,0,width/2,height/2,Math.max(width,height)*.62); bg.addColorStop(0,'rgba(15,45,66,.34)'); bg.addColorStop(1,'rgba(1,5,11,0)'); ctx.fillStyle=bg; ctx.fillRect(0,0,width,height); projected=nodes.map(project).sort((a,b)=>a.z-b.z); const pm=new Map(projected.map(p=>[p.node.id,p])); for(const l of links){{ const a=pm.get(l.source),b=pm.get(l.target); if(!a||!b) continue; const active=selected&&(selected.id===l.source||selected.id===l.target); ctx.strokeStyle=active?'rgba(50,220,255,.62)':l.kind==='task'?'rgba(167,139,250,.22)':'rgba(78,207,255,.15)'; ctx.lineWidth=active?1.8:l.kind==='task'?1.25:.9; ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(b.x,b.y); ctx.stroke(); }} for(const p of projected){{ const n=p.node,r=Math.max(3.5,n.radius*p.scale),active=selected&&selected.id===n.id,keyword=n.type==='keyword'; const glowRadius=keyword?(active?3.1:2.35):(active?4.8:n.late?3.9:3.0); const glow=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,r*glowRadius); glow.addColorStop(0,rgba(n.color,active ? (keyword ? .36 : .58) : (keyword ? .2 : .34))); glow.addColorStop(.42,rgba(n.color,active ? (keyword ? .12 : .2) : (keyword ? .07 : .1))); glow.addColorStop(1,rgba(n.color,0)); ctx.fillStyle=glow; ctx.beginPath(); ctx.arc(p.x,p.y,r*glowRadius,0,Math.PI*2); ctx.fill(); }} for(const p of projected){{ const n=p.node,r=Math.max(3.5,n.radius*p.scale),keyword=n.type==='keyword'; ctx.fillStyle=rgba(n.color,keyword ? .9 : .92); ctx.beginPath(); ctx.arc(p.x,p.y,r,0,Math.PI*2); ctx.fill(); if(keyword){{ ctx.strokeStyle=n.source==='tag'?'rgba(255,232,190,.74)':'rgba(216,204,255,.72)'; ctx.lineWidth=1.2; ctx.beginPath(); ctx.arc(p.x,p.y,r*1.62,0,Math.PI*2); ctx.stroke(); }} else {{ ctx.strokeStyle='rgba(196,250,255,.88)'; ctx.lineWidth=1.25; ctx.beginPath(); ctx.arc(p.x,p.y,r*1.18,0,Math.PI*2); ctx.stroke(); }} const style=keyword?keywordLabelStyle(n.source,p.scale):{{font:`850 ${{Math.max(10.5,12*p.scale)}}px Microsoft YaHei UI`,fill:'rgba(210,250,255,.95)'}}; ctx.font=style.font; ctx.fillStyle=style.fill; ctx.textAlign='center'; ctx.fillText(keyword?n.word:n.title.slice(0,9),p.x,p.y+r+15); }} }}
